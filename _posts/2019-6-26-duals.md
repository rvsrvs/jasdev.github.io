...
How have \(Reactive\|Rx\)Swift been around for at least four years and still not become the majority tool of choice for building applications? The answer isn’t straightforward—still, we can list a few culprits:
...
Even wilder is that, despite this duality, language design often has a bias towards one side of the dual. e.g. structs can be defined anonymously with tuples and enumerations…can’t. Properties of structs can be accessed by dot-syntax or key-paths, whereas associated values in enumerations [have to be verbosely pattern matched](https://www.pointfree.co/episodes/ep52-enum-properties).
...
The duals we’ve covered—products and co-products, effects and co-effects, and lens and prisms—are primers in seeing the duality between imperative and reactive programming. To start,  here are abbreviated definitions of our friends, and fundamental imperative types, [IteratorProtocol](https://developer.apple.com/documentation/swift/iteratorprotocol) and [Sequence](https://developer.apple.com/documentation/swift/sequence):
...
And that’s okay. If premodern programming leaned imperatively, then maybe postmodern programming is recognizing the reactive dual and knowing when to lean in the other direction.
