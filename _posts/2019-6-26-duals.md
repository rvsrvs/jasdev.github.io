...
Rix’s talk is worth multiple reads and its core is this: declarative programming—with reactive handling of data flow—allows us to tell our programs “what” to do, instead of meticulously instructing them “how“ to execute. We can then focus on the relationships between components and lean on runtimes, frameworks, and the like to determine the mechanics of how those relationships are honored.
...
This isn’t to say post-modern programming will be entirely declarative. There can be times when the dualizing the other direction makes code more performant or use a smaller memory footprint if the engine backing the declarative approach isn’t entirely sure how to optimize for a use case.

And that’s okay. If pre-modern programming leaned almost entirely imperatively. Then maybe post-modern programming is recognizing the dual that is reactive programming and knowing when to flip the coin.
